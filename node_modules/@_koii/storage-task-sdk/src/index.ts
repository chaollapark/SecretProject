import {Connection, PublicKey, LAMPORTS_PER_SOL, Keypair} from '@_koii/web3.js';

import {RawTaskData, FileMetadata, FileUploadResponse} from './Interfaces';
import fs from 'fs';
...
export class KoiiStorageClient {
  ...
  constructor(storageTaskId?: string, k2URL = 'https://testnet.koii.network', debug = false) {
  ...
  async getFile(cid: string, filename: string): Promise<Blob> {
    ...
    if (!response || !response.ok) throw new Error('Cannot fetch the file: ' + response?.statusText);
