...
import { coerce, instance, string, tuple, literal, any, unknown, union, type, optional, number, array, nullable, create, boolean, record, assert as assert$7 } from 'superstruct';
...
const ATTENTION_INSTRUCTION_LAYOUTS = Object.freeze({
  RegisterRecipient: {
    index: 0,
    layout: BufferLayout.struct([BufferLayout.u32('instruction'), // Layout.rustString("recipient"),
    // BufferLayout.ns64("shares"),
    publicKey('owner')])
  },
  SubmitPorts: {
    index: 1,
    layout: BufferLayout.struct([BufferLayout.u32('instruction'), publicKey('newAuthorized')])
  },
  Voting: {
    index: 2,
    layout: BufferLayout.struct([BufferLayout.u32('instruction'), BufferLayout.ns64('is_valid')])
  },
  Withdraw: {
    index: 3,
    layout: BufferLayout.struct([BufferLayout.u32('instruction'), BufferLayout.ns64('lamports')])
...
const BufferFromRawGetAccountData = coerce(any(), RawAccountDataResult, value => Buffer.from(value[0], 'base64'));
const BufferFromRawTaskAccountData = coerce(any(), RawAccountDataResult, value => Buffer.from(value[0], 'base64'));
/**
...
function createRpcClient(url, useHttps, httpHeaders, fetchMiddleware, disableRetryOnRateLimit, fetchFn) {
  ...
  if (fetchMiddleware) {
    fetchWithMiddleware = async (url, options) => {
      ...
      return fetchFn ? await fetchFn(...modifiedFetchArgs) : await fetch(...modifiedFetchArgs);
    };
  }
    ...
    try {
      ...
      for (;;) {
        if (fetchWithMiddleware) {
          ...
          res = fetchFn ? await fetchFn(url, options) : await fetch(url, options);
        }
...
const GetAccountInfoResult = type({
  executable: boolean(),
  owner: PublicKeyFromString,
  lamports: number(),
...
const TaskStateResult = type({
  data: BufferFromRawTaskAccountData
});
const TaskStateRoundResult = type({
  data: any()
});
/**
 * @internal
...
class Connection {
  ...
  async getAccountInfoAndContext(publicKey, encoding, commitment) {
    ...
    const res = create(unsafeRes, jsonRpcResultAndContext(nullable(GetAccountInfoResult)));
  ...
  async getTaskAccountInfoAndContext(publicKey, is_submission_required, is_distribution_required, is_available_balances_required, is_stake_list_required, encoding, commitment) {
    ...
    const res = create(unsafeRes, jsonRpcResultAndContext(nullable(TaskStateResult)));
  ...
  async getMyTaskSubmissionRoundInfoAndContext(publicKey, account_address, round, commitment) {
    ...
    const res = create(unsafeRes, jsonRpcResultAndContext(nullable(TaskStateRoundResult)));
