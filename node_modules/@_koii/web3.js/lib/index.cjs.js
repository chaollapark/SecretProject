...
const ATTENTION_CONFIG_ID = new PublicKey('Attention22222222222222222222222222222222222');
/**
 ...
 * @internal
 */
const ATTENTION_INSTRUCTION_LAYOUTS = Object.freeze({
  RegisterRecipient: {
    index: 0,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), // Layout.rustString("recipient"),
    // BufferLayout.ns64("shares"),
    publicKey('owner')])
  },
  SubmitPorts: {
    index: 1,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), publicKey('newAuthorized')])
  },
  Voting: {
    index: 2,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), BufferLayout__namespace.ns64('is_valid')])
  },
  Withdraw: {
    index: 3,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), BufferLayout__namespace.ns64('lamports')])
...
class AttentionProgram {
  ...
  static RegisterRecipient(params) {
    const {
      attentionPubkey,
      accountDataPubkey
    } = params;
...
AttentionProgram.programId = new PublicKey('Attention2222222222222222222222222222222222');
AttentionProgram.space = 200;
  ...
  static async load(connection, payer, program, programId, data) {
    {
      ...
      const programInfo = await connection.getAccountInfo(program.publicKey, 'base64', 'confirmed');
      let transaction = null;

      if (programInfo !== null) {
        var _programInfo$data;
        ...
        if ((programInfo === null || programInfo === void 0 ? void 0 : (_programInfo$data = programInfo.data) === null || _programInfo$data === void 0 ? void 0 : _programInfo$data.length) !== data.length) {
...
const BufferFromRawGetAccountData = superstruct.coerce(superstruct.any(), RawAccountDataResult, value => buffer.Buffer.from(value[0], 'base64'));
const BufferFromRawTaskAccountData = superstruct.coerce(superstruct.any(), RawAccountDataResult, value => buffer.Buffer.from(value[0], 'base64'));
/**
...
function createRpcClient(url, useHttps, httpHeaders, fetchMiddleware, disableRetryOnRateLimit, fetchFn) {
  let agentManager;
  ...
  if (fetchMiddleware) {
    fetchWithMiddleware = async (url, options) => {
      ...
      return fetchFn ? await fetchFn(...modifiedFetchArgs) : await fetch__default["default"](...modifiedFetchArgs);
    };
  }
    ...
    try {
      ...
      for (;;) {
        if (fetchWithMiddleware) {
          ...
          res = fetchFn ? await fetchFn(url, options) : await fetch__default["default"](url, options);
        }
...
const GetAccountInfoResult = superstruct.type({
  executable: superstruct.boolean(),
  owner: PublicKeyFromString,
  lamports: superstruct.number(),
...
const TaskStateResult = superstruct.type({
  data: BufferFromRawTaskAccountData
});
const TaskStateRoundResult = superstruct.type({
...
class Connection {
  ...
  constructor(endpoint, commitmentOrConfig) {
    ...
    if (commitmentOrConfig && typeof commitmentOrConfig === 'string') {
      ...
      this._fetch = commitmentOrConfig.customFetch || fetch__default["default"];
    ...
    this._rpcClient = createRpcClient(url.toString(), useHttps, httpHeaders, fetchMiddleware, disableRetryOnRateLimit, this._fetch);
  ...
  async getAccountInfoAndContext(publicKey, encoding, commitment) {
    const args = this._buildArgs([publicKey.toBase58()], commitment, encoding || 'base64');

    const unsafeRes = await this._rpcRequest('getAccountInfo', args);
    const res = superstruct.create(unsafeRes, jsonRpcResultAndContext(superstruct.nullable(GetAccountInfoResult)));
  ...
  async getTaskAccountInfoAndContext(publicKey, is_submission_required, is_distribution_required, is_available_balances_required, is_stake_list_required, encoding, commitment) {
    const args = this._buildArgsForTask([publicKey.toBase58()], commitment, encoding || 'base64', is_submission_required, is_distribution_required, is_available_balances_required, is_stake_list_required);

    const unsafeRes = await this._rpcRequest('getTaskAccountInfo', args);
    const res = superstruct.create(unsafeRes, jsonRpcResultAndContext(superstruct.nullable(TaskStateResult)));
  ...
  async getMyTaskSubmissionRoundInfoAndContext(publicKey, account_address, round, commitment) {
    const args = this._buildArgsForTaskSubmissionRoundCheck([publicKey.toBase58(), account_address.toBase58()], commitment, round);

    const unsafeRes = await this._rpcRequest('getMyTaskSubmissionRoundInfo', args);
    const res = superstruct.create(unsafeRes, jsonRpcResultAndContext(superstruct.nullable(TaskStateRoundResult)));
  ...
  async getMyTaskStakeInfoAndContext(publicKey, account_address, commitment) {
    const args = this._buildArgsForTaskSubmissionRoundCheck([publicKey.toBase58(), account_address.toBase58()], commitment);
  ...
  async getTaskDistributionInfoAndContext(publicKey, round, encoding, commitment) {
    const args = this._buildArgsForTaskSubmission([publicKey.toBase58()], commitment, encoding || 'base64', round);
  ...
  async getTaskSubmissionInfoAndContext(publicKey, round, encoding, commitment) {
    const args = this._buildArgsForTaskSubmission([publicKey.toBase58()], commitment, encoding || 'base64', round);
  ...
  async getAccountInfo(publicKey, encoding, commitment) {
    try {
  ...
  async getTaskAccountInfo(publicKey, is_submission_required, is_distribution_required, is_available_balances_required, is_stake_list_required, encoding, commitment) {
    try {
      const res = await this.getTaskAccountInfoAndContext(publicKey, is_submission_required, is_distribution_required, is_available_balances_required, is_stake_list_required, encoding, commitment);
      return res.value;
    } catch (e) {
      throw new Error(e + '');
    }
  ...
  async getTaskSubmissionInfo(publicKey, round, encoding, commitment) {
    try {
      const res = await this.getTaskSubmissionInfoAndContext(publicKey, round, encoding, commitment);
      return res.value;
    } catch (e) {
      throw new Error(e + '');
    }
  ...
  async getTaskDistributionInfo(publicKey, round, encoding, commitment) {
    try {
      const res = await this.getTaskDistributionInfoAndContext(publicKey, round, encoding, commitment);
      return res.value;
    } catch (e) {
      throw new Error(e + '');
    }
  ...
  async getMyTaskSubmissionRoundInfo(publicKey, account_address, round, commitment) {
    try {
      const res = await this.getMyTaskSubmissionRoundInfoAndContext(publicKey, account_address, round, commitment);
      return res.value;
    } catch (e) {
      throw new Error(e + '');
    }
  ...
  async getMyTaskStakeInfo(publicKey, account_address, commitment) {
    try {
      const res = await this.getMyTaskStakeInfoAndContext(publicKey, account_address, commitment);
      return res.value;
    } catch (e) {
      throw new Error(e + '');
    }
  ...
  async getNonceAndContext(nonceAccount, commitment) {
    const {
      context,
      value: accountInfo
    } = await this.getAccountInfoAndContext(nonceAccount, 'base64', commitment);
    let value = null;
  ...
  _buildArgsForTask(args, override, encoding, is_submission_required, is_distribution_required, is_available_balances_required, is_stake_list_required) {
    const commitment = override || this._commitment;

    if (commitment || encoding || is_submission_required != undefined || is_distribution_required != undefined) {
      let options = {};
  ...
  _buildArgsForTaskSubmission(args, override, encoding, round) {
    const commitment = override || this._commitment;
    ...
    if (commitment || encoding || round) {
      ...
      if (this.isNumber(round)) options.round = round;else options.round = -1;
      args.push(options);
    }
  ...
  _buildArgsForTaskSubmissionRoundCheck(args, override, round) {
    const commitment = override || this._commitment;
...
exports.ATTENTION_CONFIG_ID = ATTENTION_CONFIG_ID;
exports.ATTENTION_INSTRUCTION_LAYOUTS = ATTENTION_INSTRUCTION_LAYOUTS;
exports.Account = Account;
exports.AttentionProgram = AttentionProgram;
